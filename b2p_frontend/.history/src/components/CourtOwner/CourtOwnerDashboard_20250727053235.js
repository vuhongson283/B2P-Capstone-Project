...
const OwnerDashboard = () => {
  ...
  const handleExportExcel = async () => {
    ...
    try {
      ...
      // Kiểm tra xem response có phải là dữ liệu Excel hợp lệ
      ...
      // Tạo URL và download với tên file cụ thể hơn
      const url = window.URL.createObjectURL(blob);
      const link = document.createElement('a');
      link.href = url;
      const fileName = `bao_cao_don_dat_san_${new Date().toISOString().slice(0,10)}.xlsx`;
