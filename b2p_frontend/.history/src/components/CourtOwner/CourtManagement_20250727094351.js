...
import { getAllCourts, addNewCourt, updateCourt, deleteCourt } from '../../services/apiService';
  ...
  const handleUpdateCourt = async () => {
    try {
      ...
      const courtData = {
        ...
        statusId: isNaN(parseInt(editCourt.statusId)) ? 1 : parseInt(editCourt.statusId),
              ...
              <tbody>
                {courts.map(court => (
                  <tr key={court.courtId}>
                    ...
                    <td>
                      <span className={`status ${court.statusId === 'Active' ? 'active' : 'inactive'}`}>
                        {court.statusId === 'Active' ? 'Hoạt động' : 'Không hoạt động'}
                      </span>
                    </td>
                    <td>
