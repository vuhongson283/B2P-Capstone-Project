    ...
    try {
      ...
      if (response && response.data) {
        if (response.data.success) {
          // Set courts data từ response
          setCourts(response.data.data?.items || []);
          
          // Set pagination data từ response
          setPagination(prev => ({
            ...prev,
            pageNumber: response.data.data?.currentPage || prev.pageNumber,
            totalItems: response.data.data?.totalItems || 0,
            totalPages: response.data.data?.totalPages || 0
          }));
          
          // Set facility info từ mock data - có thể thay bằng API call thực tế
          setCurrentFacility({
            id: facilityId,
          ...
          // Trường hợp success = false (không có sân)
          if (response.data.message && response.data.message.includes('không tồn tại sân')) {
            setCourts([]);
            setPagination(prev => ({
              ...prev,
              totalItems: 0,
            ...
            setError(`Không thể tải dữ liệu sân: ${response.data.message || 'Lỗi không xác định'}`);
          }
        }
      } else {
      ...
      if (error.response) {
        setError(`Lỗi HTTP ${error.response.status}: ${error.response.statusText}`);
