...
const CourtManagement = () => {
  ...
  const handleAddCourt = async () => {
    try {
        ...
        alert(response.data.message || 'Thêm sân thành công!');
        } else {
        setError(response.data.message || 'Thêm sân thất bại. Vui lòng thử lại.');
        }
    } catch (error) {
        console.error('Error adding court:', error);
        setError('Có lỗi xảy ra khi thêm sân mới: ' + (error.response?.data?.message || error.message));
    }
    };
