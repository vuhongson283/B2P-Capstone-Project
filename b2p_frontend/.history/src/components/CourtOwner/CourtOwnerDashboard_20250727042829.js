...
const OwnerDashboard = () => {
  ...
  const fetchDashboardData = async () => {
    try {
      ...
      console.log("Total Report Response:", totalReportResponse); // Debug log
      console.log("Report Response:", reportResponse); // Debug log

      if (totalReportResponse.data && totalReportResponse.data.success) {
        // Sửa từ .items thành .data
        const totalData = totalReportResponse.data || {};
      ...
      if (reportResponse.data && reportResponse.data.success) {
        // Kiểm tra cả 2 cấu trúc có thể (data.items hoặc data.data.items)
        const items = reportResponse.data.data?.items || reportResponse.data.items || [];
        setDashboardData(prev => ({
          ...prev,
