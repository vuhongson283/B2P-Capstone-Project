...
import { getAllCourts, addNewCourt, updateCourt } from '../../services/apiService';
...
const userId = 6; // Assuming userId is 6 for Court Owner
  const handleUpdateCourt = async () => {
    try {
      const courtData = {
        courtId: parseInt(editCourt.courtId),
        status: parseInt(editCourt.statusId),
        courtName: editCourt.courtName,
        categoryId: parseInt(editCourt.categoryId),
        pricePerHour: parseInt(editCourt.pricePerHour)
      };
      
      const response = await updateCourt(courtData); // userId=6 sẽ được thêm trong apiService
      
      if (response.data?.success) {
        setShowEditModal(false);
        fetchCourts();
        alert(response.data.message || 'Cập nhật sân thành công!');
      } else {
        alert(response.data?.message || 'Cập nhật sân thất bại');
      }
    } catch (error) {
