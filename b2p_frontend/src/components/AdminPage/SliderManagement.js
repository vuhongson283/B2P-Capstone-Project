...
const SliderForm = ({ initialValues, onSubmit, loading }) => {
    ...
    const handleChange = ({ fileList: newFileList }) => {
        ...
        const hasNewFile = newFileList.some(file => file.originFileObj && !file.isExisting);
    ...
    const handleSubmit = async (values) => {
        ...
        try {
            ...
            let sliderId;
            if (isEdit) {
                sliderResponse = await updateSlider(initialValues.slideId, sliderPayload);
            ...
            if (hasNewImage) {
                ...
                if (newFile) {
                    try {
                        ...
                        if (isEdit && imageId) {
                            imageResponse = await updateSlideImage(imageId, newFile.originFileObj);
                        } else {
                            imageResponse = await uploadslideImage(newFile.originFileObj, sliderId);
                        }
                    ...
                    console.log('hasNewImage nhưng không tìm thấy file để upload → không lỗi');
                    imageSuccess = true;
                }
            }
            // 4. Hiển thị thông báo tổng kết
            if (imageSuccess === false) {
                message.warning(`${isEdit ? 'Cập nhật' : 'Tạo'} slider thành công, nhưng lỗi khi xử lý ảnh`);
            } else {
                message.success(`${isEdit ? 'Cập nhật' : 'Tạo'} slider${hasNewImage ? ' và ảnh' : ''} thành công`);
            }
            ...
            message.error(`${isEdit ? 'Cập nhật' : 'Tạo'} slider thất bại: ${detail}`);
        } finally {
            setSubmitting(false);
        }
    };
        ...
        Modal.confirm({
            ...
            content: (
                <ModalContent
                    ...
                    content={`Bạn có chắc chắn muốn ${isActive ? "vô hiệu hóa" : "kích hoạt"} slider này không?`}
            ...
            onOk: async () => {
                try {
                    ...
                    if (res?.status === 200) {
                        message.success(`${isActive ? "Vô hiệu hóa" : "Kích hoạt"} thành công`);
                        fetchSliders();
                    }
                } catch (e) {
        ...
        if (record.statusName === "Active") {
            Modal.warning({
                ...
                content: (
                    <>
                        <p>
                            Slider <strong>{record.slideDescription}</strong> hiện đang hoạt động.
                        </p>
            ...
            try {
                ...
                if (response?.status === 200 && response.data) {
                    ...
                    setEditingSlider({
                        ...
                        imageId: sliderDetails.imageId || sliderDetails.images?.[0]?.imageId
                    });
                } else {
        ...
        },
        {
            ...
            render: (_, record) => (
                <div className="action-buttons">
                    <Tooltip title="Sửa">
                        <EditOutlined className="edit-icon" onClick={() => openFormModal(record)} />
                    </Tooltip>
                    <Tooltip title="Xóa">
    ...
    return (
        <div className="account-table-wrapper">
            <div className="account-table-card">
                ...
                >
                    <SliderForm
                        initialValues={editingSlider || { slideDescription: "", slideUrl: "", imageUrl: "" }}
