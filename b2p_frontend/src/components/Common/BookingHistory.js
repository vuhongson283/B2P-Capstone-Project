...
const BookingHistory = () => {
  useEffect(() => {
    document.title = "Lịch sử đặt sân - B2P";
  }, []);
  ...
  const canCancelBooking = (booking) => {
    ...
    const hasTransactionCode = !!(
      booking.transactionCode || booking.rawBookingData?.transactionCode
    );
  ...
  const handleConfirmedCancel = async () => {
    ...
    try {
      ...
      const transactionCode =
        bookingToCancel.transactionCode ||
      ...
      if (!transactionCode) {
        console.error(
          "❌ [handleConfirmedCancel] Missing TransactionCode, but showing success anyway"
        );
      ...
      try {
        ...
        console.log(
          "✅ [handleConfirmedCancel] API called successfully, showing success message"
        );
      } catch (apiError) {
        console.error(
          "❌ [API ERROR] Cancel API failed but showing success anyway:",
          {
            error: apiError,
      ...
      console.error(
        "❌ [handleConfirmedCancel] Unexpected error but showing success:",
        {
          error: error,
  ...
  const showSlotsModal = (booking) => {
    ...
    const formattedSlots = (booking.rawSlotData || []).map((slot, index) => {
      ...
      return {
        ...
        timeSlot: `${slot.startTime?.substring(
          0,
          5
    ...
    const calculatedTotal = formattedSlots.reduce(
      (total, slot) => total + slot.rawPrice,
      0
    );
  ...
  const processBookingData = async (bookingsData) => {
    ...
    for (const booking of bookingsData) {
      ...
      if (booking.statusId === 8) {
        console.log(
          `🚫 [DEBUG] Skipping booking ${booking.bookingId} with statusId = 8 (UnPaid)`
        );
        continue;
      }
      ...
      if (
        ...
        if (!mappedStatus || mappedStatus === "unknown") {
          console.log(
            `🚫 [DEBUG] Skipping booking ${booking.bookingId} - Status mapping returned null or unknown`
          );
          continue;
        }
        ...
        console.log(
          `✅ [DEBUG] Processed booking (${booking.slots.length} slots combined):`,
          {
    ...
    console.log(
      `📊 [DEBUG] Filter summary: ${bookingsData.length} total → ${processedBookings.length} after processing`
    );
  ...
  const loadBookingHistory = async () => {
    ...
    try {
      ...
      console.log(
        `🚫 [DEBUG] Found ${unpaidBookings.length} bookings with statusId = 8 (UnPaid) - these will be filtered out`
      );
      ...
      if (processedBookings.length > 0) {
        loadCustomerInfoForBookings(
          processedBookings.slice(0, bookingsPerPage)
        );
      }
    } catch (error) {
    ...
    for (const booking of bookingsToLoad) {
      if (booking.userId) {
        try {
          ...
          console.error(
            "Error loading customer info for booking:",
            booking.id,
            error
          );
        }
      ...
      if (customerData) {
        return {
          customerName:
            ...
            "N/A",
          customerPhone:
            customerData.phoneNumber || customerData.phone || "N/A",
  ...
  const filteredBookings = useMemo(() => {
    return bookings.filter((booking) => {
      ...
      if (booking.statusId === 8) {
        console.log(
          `🚫 [filteredBookings] Filtering out booking ${booking.id} with statusId = 8 (UnPaid)`
        );
        return false;
      }
  ...
  useEffect(() => {
    if (currentBookings.length > 0) {
      const bookingsNeedingCustomerInfo = currentBookings.filter(
        (b) => b.customerName === "Đang tải..."
      );
    ...
    <div className="booking-history-page">
                    ...
                    <div key={booking.uniqueKey} className="booking-row">
                      <div className="booking-main">
                        ...
                        <div className="court-info">
                          ...
                          <span
                            className={`court-type type-${booking.courtType
                              .toLowerCase()
                              .replace(/\s+/g, "-")
                          ...
                          <div className="detail-item">
                            ...
                            >
                              <path
                                ...
                                strokeLinejoin="round"
                                strokeWidth={2}
                                d="M8 7V3a1 1 0 011-1h6a1 1 0 011 1v4h3a2 2 0 012 2v1l-2 13H5L3 10V9a2 2 0 012-2h3z"
                              />
                            </svg>
              ...
              <div className="empty-state">
                <div className="empty-illustration">
                  <svg viewBox="0 0 200 200" className="empty-svg">
                    ...
                    />
                    <path
                      d="M70 85h60M70 105h40M70 125h50"
                      stroke="#64b5f6"
                      strokeWidth="3"
                      strokeLinecap="round"
                    />
                ...
                <p className="empty-description">
                  Bạn chưa có lần đặt sân nào hoặc không tìm thấy kết quả phù
      ...
      >
        <div style={{ marginBottom: "16px" }}>
          <p>
            <strong>Tổng số slot:</strong> {selectedBookingSlots.length}
          </p>
          <p>
            <strong>Tổng thời lượng:</strong>{" "}
            {selectedBookingSlots.reduce((total, slot) => {
                ...
                <div className="detail-header">
                  <div className="booking-id-section">
                    <div className="booking-info-left">
                      ...
                      <span className="booking-created">
                        Đặt lúc: {formatDateTime(selectedBooking.bookingDate)}
                      </span>
                    </div>
                    <span
                      className={`status-badge ${getStatusClass(
                  ...
                  <div className="quick-info-cards">
                    ...
                    <div className="quick-card time-card">
                      ...
                      <div className="card-value">
                        {selectedBooking.totalSlots} slot
                      </div>
                    </div>
                  ...
                  <div className="detail-section">
                    ...
                    <div className="detail-grid">
                      ...
                      <div className="detail-item">
                        ...
                        <div className="value">
                          {selectedBooking.facilityContact}
                        </div>
                      </div>
                    </div>
                  </div>
                  ...
                  <div className="detail-section">
                    ...
                    <div className="detail-grid">
                      ...
                      <div className="detail-item">
                        ...
                        <div className="value court-type">
