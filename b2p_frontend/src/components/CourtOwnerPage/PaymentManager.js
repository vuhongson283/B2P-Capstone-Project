...
const PaymentManager = () => {
  ...
  const getAccountTypeDisplay = (paymentMethodName) => {
    ...
    if (lowerMethod.includes('vnpay') || lowerMethod.includes('momo') || lowerMethod.includes('zalopay')) {
      return 'Quốc nội';
    }
  ...
  const fetchPaymentAccounts = useCallback(async () => {
    ...
    try {
      ...
      if (response && response.data && Array.isArray(response.data)) {
        ...
        console.log('No data received or unexpected response structure:', response);
        setAccounts([]);
      }
    } catch (err) {
      ...
      let errorMessage = 'Không thể tải danh sách tài khoản thanh toán. Vui lòng thử lại.';
  ...
  const toggleAccountStatus = useCallback(async (id) => {
    try {
      ...
      if (isSuccess) {
        ...
        setSuccess(
          `Trạng thái tài khoản đã được cập nhật thành ${newStatusId === 1 ? 'Active' : 'Inactive'}.`
        );
        ...
        console.warn('Response không có success flag nhưng có thể đã update thành công. Refreshing data...');
        ...
        setSuccess('Trạng thái tài khoản có thể đã được cập nhật. Đang làm mới dữ liệu...');
      ...
      if (err.response?.status === 200 || err.response?.status === 201) {
        ...
        setSuccess(`Trạng thái tài khoản đã được cập nhật thành ${newStatusId === 1 ? 'Active' : 'Inactive'}.`);
        setTimeout(() => setSuccess(null), 3000);
        return;
      }
  ...
  const handleCreateAccount = useCallback(async () => {
    ...
    try {
      ...
      if (isSuccess) {
        ...
        console.warn('Response không có success flag nhưng có thể đã tạo thành công. Refreshing data...');
        ...
        setSuccess('Tài khoản thanh toán có thể đã được tạo. Vui lòng kiểm tra danh sách.');
      ...
      let errorMessage = 'Không thể tạo tài khoản thanh toán. Vui lòng thử lại.';
      ...
      if (err.response?.data?.message) {
        ...
        errorMessage = 'Payment key đã tồn tại. Vui lòng sử dụng payment key khác.';
      } else if (err.response?.status === 401) {
          ...
          <div className="table-container">
            ...
            <div className="table-content">
              {loading ? (
                ...
                <div className="table-body">
                  {accounts.map((account, index) => (
                    <div key={account.id} className="table-data-row">
                      ...
                      <div className="table-cell payment-key" title={account.paymentKey}>
                        {account.paymentKey}
                      </div>
                ...
                <div className="empty-state">
                  <div className="empty-message">
                    {error ? 'Có lỗi xảy ra khi tải dữ liệu' : 'Bạn chưa có tài khoản thanh toán. Click vào nút "Tạo tài khoản thanh toán mới" để bắt đầu.'}
                  </div>
                </div>
              )}
            </div>
            ...
            <div style={{
              ...
              <button 
                ...
                style={{
                  ...
                  border: 'none',
                  borderRadius: '6px',
                  backgroundColor: (!accountType || !paymentKey.trim() || loading) ? '#9ca3af' : '#3b82f6',
                  color: 'white',
                  cursor: (!accountType || !paymentKey.trim() || loading) ? 'not-allowed' : 'pointer',
                  fontSize: '14px'
                }}
              >
