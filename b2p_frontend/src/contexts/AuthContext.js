...
export const AuthProvider = ({ children }) => {
  ...
  useEffect(() => {
    ...
    try {
      ...
      console.error(
        "❌ AuthContext: Error loading user from localStorage:",
        error
      );
  ...
  const hasRole = (requiredRoles) => {
    ...
    console.log(
      `🔍 hasRole: User role "${user.roleName}", Required: [${roles.join(
        ", "
      )}], Access: ${hasAccess}`
    );
    return hasAccess;
  ...
  const hasRoleId = (requiredRoleIds) => {
    ...
    const roleIds = Array.isArray(requiredRoleIds)
      ? requiredRoleIds
      : [requiredRoleIds];
    const hasAccess = roleIds.includes(user.roleId);

    console.log(
      `🔍 hasRoleId: User roleId "${user.roleId}", Required: [${roleIds.join(
        ", "
      )}], Access: ${hasAccess}`
    );
    return hasAccess;
  ...
  const isAdmin = () => {
    const adminCheck =
      user?.roleName === ROLES.ADMIN || user?.roleId === ROLE_IDS.ADMIN;
    console.log(
      `👑 isAdmin: ${adminCheck} (Role: ${user?.roleName}, RoleId: ${user?.roleId})`
    );
    return adminCheck;
  };
  ...
  const isPlayer = () => {
    const playerCheck =
      user?.roleName === ROLES.PLAYER || user?.roleId === ROLE_IDS.PLAYER;
    console.log(
      `🏃‍♂️ isPlayer: ${playerCheck} (Role: ${user?.roleName}, RoleId: ${user?.roleId})`
    );
    return playerCheck;
  };

  const isCourtOwner = () => {
    const courtOwnerCheck =
      user?.roleName === ROLES.COURTOWNER ||
      user?.roleId === ROLE_IDS.COURTOWNER;
    console.log(
      `🏢 isCourtOwner: ${courtOwnerCheck} (Role: ${user?.roleName}, RoleId: ${user?.roleId})`
    );
...
export const ProtectedRoute = ({
  ...
  if (playerAndGuestOnly) {
    ...
    if (user && isAdmin() && adminRedirect) {
      console.log(
        "🔄 ProtectedRoute: Admin detected, redirecting to:",
        adminRedirect
      );
    ...
    if (user && isCourtOwner() && courtOwnerRedirect) {
      console.log(
        "🔄 ProtectedRoute: CourtOwner detected, redirecting to:",
        courtOwnerRedirect
      );
    ...
    console.log(
      "✅ ProtectedRoute: playerAndGuestOnly - Access granted for Guest or Player"
    );
    return children;
  }
  ...
  if (adminOnly && !isAdmin()) {
    console.log(
      "🚫 ProtectedRoute: Admin access required but user is not admin"
    );
  ...
  if (playerOnly && !isPlayer()) {
    console.log(
      "🚫 ProtectedRoute: Player access required but user is not player"
    );
    return <Navigate to={fallbackPath} replace />;
  }
  ...
  if (courtOwnerOnly && !isCourtOwner()) {
    console.log(
      "🚫 ProtectedRoute: CourtOwner access required but user is not court owner"
    );
    return <Navigate to={fallbackPath} replace />;
  }
  ...
  if (rolesToCheck.length > 0 && !hasRole(rolesToCheck)) {
    console.log(
      `🚫 ProtectedRoute: Required roles ${rolesToCheck.join(
        ", "
      )} not met. User role: ${user.roleName}`
    );
    return <Navigate to={fallbackPath} replace />;
  }
  ...
  if (requiredRoleIds.length > 0 && !hasRoleId(requiredRoleIds)) {
    console.log(
      `🚫 ProtectedRoute: Required role IDs ${requiredRoleIds.join(
        ", "
      )} not met. User role ID: ${user.roleId}`
    );
    return <Navigate to={fallbackPath} replace />;
  }
...
export const RoleBasedComponent = ({
  ...
  const roleIdsToCheck =
    allowedRoleIds.length > 0 ? allowedRoleIds : requiredRoleIds;
