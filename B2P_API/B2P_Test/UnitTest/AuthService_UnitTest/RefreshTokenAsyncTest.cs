...
namespace B2P_Test.UnitTest.AuthService_UnitTest
{
    public class RefreshTokenAsyncTest
    {
        ...
        // In UTCID11_Success_UserPhoneFullNameEmailNull_AndRoleNull, update the assertions for FullName and Email to expect "" instead of null
        ...
        public async Task UTCID11_Success_UserPhoneFullNameEmailNull_AndRoleNull()
        {
            ...
            // Fix for nullable reference type warnings in test setup

            // For all User properties that are non-nullable reference types, use string.Empty or new Role() instead of null.

            var user = new User
            {
                UserId = 8,
                StatusId = 1,
                Phone = string.Empty,          // Use empty string instead of null
                FullName = string.Empty,       // Use empty string instead of null
                Email = string.Empty,          // Use empty string instead of null
                IsMale = false,
                Dob = new DateOnly(2000, 1, 1),
                Address = "Test address",
                Role = null,                   // If Role is non-nullable, use: new Role { RoleName = "User" }
            ...
            Assert.Equal("", result.Data.User.FullName); // changed from null to ""
            Assert.Equal("", result.Data.User.Email);    // changed from null to ""
            Assert.Equal("User", result.Data.User.RoleName);
